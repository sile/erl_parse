...
//! `NodeView` / `Cursor` navigation surface so it matches what
//! external consumers see.

use erl_parse::{Cursor, NodeView, ParseMode, Parser, SyntaxKind, SyntaxTree};
...
/// expression grammar) and returns the completed `SyntaxTree`.
...
/// Returns a `NodeView` for the tree's first root node.
fn root_view(tree: &SyntaxTree) -> NodeView<'_> {
    Cursor::new(tree.tokens(), tree.syntax())
        .roots()
        .next()
        .expect("at least one root node")
}

fn child_kinds(view: NodeView<'_>) -> Vec<SyntaxKind> {
    view.children().map(|c| c.kind()).collect()
}

fn find_child(view: NodeView<'_>, kind: SyntaxKind) -> Option<NodeView<'_>> {
...
fn receive_with_only_clauses_has_no_after_section() {
    ...
    assert_eq!(receive.kind(), SyntaxKind::ReceiveExpr);
    assert!(!child_kinds(receive).contains(&SyntaxKind::ReceiveAfterSection));
}
...
fn receive_with_only_after_wraps_the_after_section() {
    ...
    assert_eq!(receive.kind(), SyntaxKind::ReceiveExpr);
    let after =
        find_child(receive, SyntaxKind::ReceiveAfterSection).expect("after section is present");
    // Body plus the timeout expression live under the section.
    let inner: Vec<SyntaxKind> = after.children().map(|c| c.kind()).collect();
    assert!(
        inner.contains(&SyntaxKind::Body),
        "after section holds a Body: {inner:?}"
    );
    // The section's range must start on the `after` keyword itself, so
    // it is strictly narrower than the enclosing ReceiveExpr range.
...
fn receive_with_clauses_and_after_holds_both() {
    ...
    assert!(
        kinds.contains(&SyntaxKind::Clause),
        "clauses present: {kinds:?}"
    );
    assert!(
        kinds.contains(&SyntaxKind::ReceiveAfterSection),
...
fn try_with_of_catch_after_wraps_each_section() {
    ...
    assert_eq!(try_node.kind(), SyntaxKind::TryExpr);
    let kinds = child_kinds(try_node);
    assert!(
        kinds.contains(&SyntaxKind::TryOfSection),
        "of section: {kinds:?}"
    );
    assert!(
        kinds.contains(&SyntaxKind::TryCatchSection),
        "catch section: {kinds:?}"
    );
    assert!(
        kinds.contains(&SyntaxKind::TryAfterSection),
        "after section: {kinds:?}"
    );
}
...
fn try_of_clauses_and_pattern_only_catch_clauses_are_distinguishable() {
    ...
    let try_node = root_view(&tree);
    let of_section = find_child(try_node, SyntaxKind::TryOfSection).expect("of section");
    let catch_section = find_child(try_node, SyntaxKind::TryCatchSection).expect("catch section");
    assert!(
        of_section
            .children()
            .any(|c| c.kind() == SyntaxKind::Clause)
    );
    assert!(
        catch_section
            .children()
...
fn try_with_after_only_omits_of_and_catch_sections() {
    ...
    let try_node = root_view(&tree);
    let kinds = child_kinds(try_node);
    assert!(!kinds.contains(&SyntaxKind::TryOfSection));
    assert!(!kinds.contains(&SyntaxKind::TryCatchSection));
    assert!(kinds.contains(&SyntaxKind::TryAfterSection));
}
...
fn try_catch_section_holds_both_clause_and_catch_clause_shapes() {
    ...
    assert!(inner.contains(&SyntaxKind::Clause), "{inner:?}");
...
fn maybe_without_else_has_no_else_section() {
    ...
    assert_eq!(maybe.kind(), SyntaxKind::MaybeExpr);
    assert!(!child_kinds(maybe).contains(&SyntaxKind::MaybeElseSection));
}
...
fn maybe_with_else_wraps_the_else_section() {
    ...
    let maybe = root_view(&tree);
    let else_section =
        find_child(maybe, SyntaxKind::MaybeElseSection).expect("else section is present");
...
fn section_ranges_do_not_include_the_enclosing_end_keyword() {
    ...
    for kind in [
        SyntaxKind::TryOfSection,
        SyntaxKind::TryCatchSection,
        SyntaxKind::TryAfterSection,
    ] {
